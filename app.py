...
#PAGE SETUP
st.set_page_config(page_title="DONATIONS DASHBOARD", page_icons="📊", layout="wide")
...
st.sidebar.caption("Expected columns: donor_name, data, amount, campaign, payment_method")
...
with left:
    ...
    monthly = df.grouby("month")["amount"].sum()
...
with right:
    ...
    top_donors = {
        df.groupby("donor_name")["amount"].sum().sort_values(acsending=False).head(10).reset_index()
    }
...
def build_summary(df):
    ...
    first_gift = df.groupby("donor_name")["data"].min()
